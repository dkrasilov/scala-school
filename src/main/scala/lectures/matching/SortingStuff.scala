...
object SortingStuff extends App {
  ...
  val stuff = List(
    Boots("Adidas", 42),
    Book("Game of thrones 1", isInteresting = true),
    Boots("Reebok", 42),
    ...
    Book("Game of thrones 2", isInteresting = true),
    Book("Idiot", isInteresting = true),
  ...
  private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox) = item match {
    ...
    case it@Boots(brand, _) if brand == "Adidas" || brand == "Converse" => stuffBox.copy(boots = it :: stuffBox.boots)
    case it@Book(_, isGood) if isGood => stuffBox.copy(books = it :: stuffBox.books)
    case junk@_ => stuffBox.copy(junk = junk :: stuffBox.junk)
  }
  //
  def findMyKnife(stuffBox: StuffBox): Boolean = {
    def findInJunk(junk: List[Stuff]): Boolean = junk match {
      case Nil => false
      case ::(head, tl) => head match {
        case Knife => true
        case _ => findInJunk(tl)
      }
    }

    findInJunk(stuffBox.junk)
  }
