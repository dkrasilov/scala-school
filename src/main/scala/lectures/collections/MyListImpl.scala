...
object MyListImpl extends App {
  ...
  case class MyList(data: List[Int]) {
    ...
    def map(f: (Int) => Int): MyList = flatMap(elem => MyList(List(f(elem))))

    def foldLeft(acc: Int)(f: (Int, Int) => Int): Int = data match {
      case Nil => acc
      case ::(head, tl) => MyList(tl).foldLeft(f(acc, head))(f)
    }

    def filter(p: Int => Boolean): MyList = flatMap(elem => if (p(elem)) MyList(List(elem)) else MyList(Nil))
  }
